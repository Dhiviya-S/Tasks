...
o=[]
for num in a: #Each number in 'a' list will be passed
...
for number in a:
    ...
    while(num>=10): #To repeat the inner loop if num value is greater than 10.Loop repeats until sum reaches 1
        ...
        while (num > 0): #Checks whether the given value is greater than zero(n=100)
            ...
            sum_m = sum_m + (n * n) #Sum of the square of the digit [0+(0*0)]
...
for number in itertools.chain(a,b,c): #To iterate through a,b and c list we use itertools.chain()
...
new_list=list(s) #List without any duplicates
non=[]
for num in new_list:
...
for lis in range(rating):
    ...
    a.append(lis) #Each element will be appended to new list 'a'
print(a)
a.sort() #Sorts in ascending order
print("The minimum element in the given list:",a[0])#using indexing to find minimum element
print()
##TRIPLET IN LIST WHOSE SUM=59##
a=[10,20,30,9]
for i in range(0,4):#loops through i=0 to 3
   for j in range(1,4):#loops through j=0 to 3
            for k in range(2, 4):#loops through k=2 to 3
                if (a[i] + a[j] + a[k] == 59): #In each looping it checks sum=59
                    result=(a[i], a[j], a[k]) #prints three values which sums to 59
